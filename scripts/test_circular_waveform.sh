...
ffmpeg -y -loop 1 -i assets/1200x1200bf.webp -i assets/test_audio.mp3 -filter_complex "
...
[wave_color]pad=1200:1200:0:(555-150)[wave_centered];
[wave_centered]geq=lum='between(sqrt((X-600)*(X-600)+(Y-555)*(Y-555)),241+1.5*((lum(((-atan2(Y-555,X-600)+PI)/(2*PI))*(W-1),555,1)-128)/128)-2,241+1.5*((lum(((-atan2(Y-555,X-600)+PI)/(2*PI))*(W-1),555,1)-128)/128)+2)*255'[ring_raw];
[ring_raw]gblur=sigma=8[ring_glow];
[bg][ring_glow]overlay=0:0[outv]
" -map "[outv]" -map 1:a -c:v libx264 -preset veryfast -crf 18 -c:a aac -b:a 192k -shortest output_test.mp4
