...
def render_video(audio, output, episode_title=None, season_label=None):
    ...
    filter_complex = (
        ...
        f"drawtext=fontfile={FONT_FILE}:text='{safe_season_label}':x=(w-text_w)/2:y=120:fontsize=32:fontcolor=gold;[bg_season]"
        f"drawtext=fontfile={FONT_FILE}:text='{safe_episode_title}':x=(w-text_w)/2:y=180:fontsize=30:fontcolor=white;[bg_ep]"
        f"drawtext=fontfile={FONT_FILE}:text='{safe_ticker_text}':x=w-mod(t*120\\\\,w+text_w):y=h-60:fontsize=26:fontcolor=white;[final]"
